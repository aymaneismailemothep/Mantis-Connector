...
module.exports = {
  ...
  name: "monitor_an_issue",
  title: "Monitor an issue",
  description: "",
  version: "v1",
  input: {
    title: "Monitor an issue",
  ...
  execute: function (input, output) {
    ...
    const url = new URL(`${mantisUrl}/api/rest/issues/${issueId}/monitor`);
    const options = {
      method: 'POST',
      ...
      res.on('end', () => {
        try {
          if (res.statusCode === 401) {
            ...
            output(null, { result: `Successfully started monitoring issue ${issueId}.` });
          } else {
